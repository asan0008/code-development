...
## Oscillatory Shear Flow: OSCSHEAR = 1 and all other = 0
## Only Elongational Flow: SHEAR = 0 and ELONGATION = 1
## Mixed Flow: SHEAR = 1 and ELONGATION = 1
SHEAR = 0
OSCSHEAR = 1
ELONGATION = 0 
EQinEQ = 0 # 1 if equilibration is to be done without flow and HI nd 0 if flow has to happen (with HI)
#######################
...
##################################################################################################
cbycstar = 0.1  # c/c*
Nbpc = 24 # Number of beads per chain
a = 0.53705 # Non-dimensional bead radius (a = dim. bead rad/l_H = (h^*)*\sqrt{\pi})
springType = 0 #0: Hookean, 1:FENE, 2: ILC, 3:WLC, 4:FENE30 (with spring constant = 30), 5: Fraenkel
b_para = 50.0  # FENE b parameter OR |Q0|^2
Q_para = 1.0 # Q parameter for Fraenkel spring
#####################
...
#################################################################################################	
Chi = 0.0  # Mixedness parameter (0: PSF and OSF, 1: PEF, -1: pure rotation flow)
Gamma = 0.2 # in case of Oscillatory shear this is \gamma_0, where \gamma_0 = tan(\alpha)
omega = 0.8 # frequency of oscillation in OSF
...
gam_dot = (1-Chi)*Gamma # Shear rate or amplitude for OSF
if ELONGATION == 1:
...
######################
typeAS = 0  #1 for ASSOCIATION, 0 for NO ASSOCIATION
...
######################
typeHI = 1 # 1 for Hydrodynamic interaction 2 for without hydrodynamic interaction
## Ewald parameter
...
######################
equi_time = 39.27
delT_Eq = 0.005                               # time step size
N_steps_Eq = int(equi_time/delT_Eq)            # Number of time steps
#N_steps_Eq = 1
data_interval_file_Eq = N_steps_Eq/6
data_interval_screen_Eq = N_steps_Eq/6     # After how many data points do you want to print on the screen?
######################
## PRODUCTION
######################
pro_time = 23.56
delT_Pro = 0.005                             # time step size
N_steps_Pro = int(pro_time/delT_Pro)           # Number of time steps
#N_steps_Pro = 20
data_interval_file_Pro = N_steps_Pro/124  # After how many data points do you want to print in data file?
...
## FILE NAMES
###################################
file_tag = "Gamma%sChi%sNb%sNc%sdt%s%sb%sphi%sP%s" % (Gamma, Chi, Nbpc, Nc, delT_Pro, spring, b_para, phi, periodicity)
...
# INPUT
#cdffile = '/short/g16/as6030/traj_f5cbycstr0.8/r%i%s.nc' % (run_number, file_tag) #The name of the .nc file
cdffile = 'traj/r%i%s.nc' % (run_number, file_tag) #The name of the .nc file
